...
class HelloWorld():
    ...
    def index(self,**kwargs):
        ...
        elif cherrypy.url() == 'https://carlozanieri.it/' :
           tmpl = env.get_template('mytemplate.html')
           if kwargs :
                pag = kwargs['pag']
           else :
    ...
    @cherrypy.expose
    def menu(self,luogo="index"):
        tmp=env.get_template('mytemplate.html')
        streamValue=tmp.generate()
        return tmp.render(salutation='Menu', pag="nivo-news",target='World', blogs=Connect.blog(""),menu=Connect.menu(""), submenu=Connect.submnu(""),pagina=Connect.body("", "index"), slider=Connect.slider("", luogo), renderer="json")
    
    @cherrypy.expose
    def manifesta(self):
        tmp=env.get_template('Cross/manifesta.html')
        streamValue=tmp.generate()
        return tmp.render(salutation='Menu', target='World', blogs=Connect.blog(""),manifesta=Connect.manifesta(""), menu=Connect.menu(""), submenu=Connect.submnu(""),pagina=Connect.body("", "index"), renderer='json')
    ...
    def slide(self, luogo):
        ...
        return tmp.render(menu=Connect.menu(""), submenu=Connect.submnu(""),luogo = "sanpiero", slider=Connect.slider("", luogo), renderer="json")
    ...
    def news_one(request,titolo,id):
        ...
        #id=request.POST['id']
        return tmp.render(menu=Connect.menu(""), submenu=Connect.submnu(""),pagina=Connect.body("", "sanpiero"), manifestazione="Blog", news=Connect.news_one("",titolo, id))
    ...
    def blogs_one(self,titolo,blogid, path=True, **kwargs):
        ...
        return path
    @cherrypy.expose
    def manifesta_one(self,titolo,blogid, path=True, **kwargs):
        #titolo=request.POST['titolo']
        ...
        if not path:
          ...
          raise cherrypy.HTTPRedirect(cherrypy.url('/?%s%s' % ("pag=blogs_one&blogid=",blogid)))
            ...
            tmp=env.get_template('manifesta_one.html')
            path= tmp.render(pagina=Connect.body("", "sanpiero"), manifestazione="Blog", blogo=Connect.blogs_one("",blogid),pag="blogs_one",blogid=blogid,menu=Connect.menu(""), submenu=Connect.submnu(""))
        return path
    ...
    def chisiamo(self):
        tmpl = env.get_template('mytemplate.html')
        return tmpl.render(salutation='Hello', target='World',  menu=Connect.menu(""), submenu=Connect.submnu(""),pagina=Connect.body("", "chisiamo"), luogo="chisiamo")

    @cherrypy.expose
    def newss(self,*url_parts, **params):
                
        tmp=env.get_template('news.html')
        return tmp.render(menu=Connect.menu(""), submenu=Connect.submnu(""), pagina=Connect.body("", "sanpiero"), manifestazione="news", news=Connect.news(""), urlx="by Carlo Zanieri")
    ...
    def store_mp3_view(request):
        ...
        return tmp.render(menu=Connect.menu(""), submenu=Connect.submnu(""), pagina=Connect.body("", "sanpiero"), manifestazione="news", news=Connect.news(""))
    ...
    @cherrypy.expose
    def news(self, luogo="mugello"):
        tmp=env.get_template('nivomaster.html')
        streamValue=tmp.generate()
        
        #luogo = request.params[0]

        return tmp.render(menu=Connect.menu(""), submenu=Connect.submnu(""),luogo = "sanpiero", pagina=Connect.body("", "sanpiero"),slider=Connect.slider("", luogo), news=Connect.news(""),renderer="json")

    @cherrypy.expose
    def newsn(self, luogo="mugello"):
        tmp=env.get_template('nivo-news.html')
        streamValue=tmp.generate()
...
''' .format(upload_path + "/" + file.filename, size, file.content_type, data)
    ...
    def ins_manifestazioni(request):
        ...
        return tmp.render( menu=Connect.menu(""), submenu=Connect.submnu(""),pagina=Connect.body("", "sanpiero"), manifestazione="news", news=Connect.news(""), directory="./static/manifestazioni/img/" , dirdb="/manifestazioni/img/",tipo="manifestazioni")

    @cherrypy.expose
    def ins_news(request):
       
        tmp=env.get_template('inserimenti.html')
        return tmp.render(menu=Connect.menu(""),blogs=Connect.blog(""), submenu=Connect.submnu(""), pagina=Connect.body("", "sanpiero"), manifestazione="news", news=Connect.news(""), directory="./static/news/img/",dirdb="/news/img/" ,tipo="news")
    ...
    def manifestax(self):
        ...
        tmp=env.get_template('Cross/manifesta.html')
        return tmp.render( menu=Connect.menu(""), submenu=Connect.submnu(""),pagina=Connect.body("", "sanpiero"), manifestazione="manifesta", manifesta=Connect.blog(""),renderer='json')
