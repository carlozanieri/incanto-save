...
class Connect:
    ...
    define("mysql_host", default="carlozanieri.it", help="incanto database host")
    define("mysql_database", default="incanto", help="incanto database name")
    define("mysql_user", default="root", help="incanto database user")
    define("mysql_password", default="trex39", help="incanto database password")
    ...
    def blog(self):
        ...
        ##cursor.execute("SELECT *  from slider")
        rows = cursor.fetchall()
        blogs = [dict(id=row[0], title=row[2], dir=row[8], img=row[7],img2=row[11], html=row[4], html2=row[9],date=row[6]) for row in rows]
        # menu = primanota[1]["descrizione"]
        return blogs
    ...
    def manifesta(self):
        ...
        cursor = db.cursor()
        cursor.execute("SELECT *  from manifestazioni ")
